...
EVENT_TYPE = SOURCE_TYPE_NAME = 'scadadog'


def convert_value(in_key, in_val, db_name):
    key = "scadadog.%s" % in_key
    val = in_val['value']
    return {key: val}
...
def convert_query_speed(in_key, in_val, dn_name):
    ...
    for ent in entry_key:
        new_key = "scadadog.%s.%s" % (in_key, ent)
    ...
    def check(self, instance):
        try:
            ...
            response = requests.get(
                instance['scadadog_url'] + '/admin/status', headers={'Authorization': 'Basic {}'.format(auth_token)}
            )
        except KeyError:
            raise Exception('The scadadog check instance is not properly configured')
        ...
        try:
            tags = instance['tags']
            if type(tags) != list:
                self.log.warning('The tags list in the scadadog check is not configured properly')
                tags = []
        except KeyError:
            tags = []

        tags.append("scadadog_url:%s" % instance['scadadog_url'])
